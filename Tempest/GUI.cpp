...
// init renderer to nullptr
CEGUI::OpenGL3Renderer* Tempest::GUI::m_renderer = nullptr;


namespace Tempest {
	
	// ---------- functions to convert sdl input to cegui input ------------
	...
	void GUI::init(const std::string & resourceDirectory) {
		...
		if (m_renderer == nullptr) {
			...
			CEGUI::DefaultResourceProvider* provider = static_cast<CEGUI::DefaultResourceProvider*>(CEGUI::System::getSingleton().getResourceProvider());
			provider->setResourceGroupDirectory("imagesets", resourceDirectory + "/imagesets/");
			provider->setResourceGroupDirectory("schemes", resourceDirectory + "/schemes/");
			provider->setResourceGroupDirectory("fonts", resourceDirectory + "/fonts/");
			provider->setResourceGroupDirectory("layouts", resourceDirectory + "/layouts/");
			provider->setResourceGroupDirectory("looknfeels", resourceDirectory + "/looknfeel/");
			provider->setResourceGroupDirectory("lua_scripts", resourceDirectory + "/lua_scripts/");
	...
	void GUI::draw() {
		...
		if (!(m_renderer == nullptr)) {
			// before rendering depth test needs disabling

			// before rendering depth test needs disabling
			glDisable(GL_DEPTH_TEST);
			// disable scissor test and other flags that can cause problems with rendering UI
	...
	CEGUI::Window * GUI::createWidget(
		...
		CEGUI::Window* widgetWindow = CEGUI::WindowManager::getSingleton().createWindow(type, name);
		m_root->addChild(widgetWindow);
		setWidgetDestRect(widgetWindow, destRectPercentage, destRectPixel);
	...
	CEGUI::Window * GUI::createWidget(
		...
		setWidgetDestRect(newWindow, destRectPercentage, destRectPixel);
		return newWindow;
